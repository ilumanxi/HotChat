...
extension KeychainStore {
    ...
    func set(_ string: String, for key: String) throws {
        guard let data = string.data(using: .utf8, allowLossyConversion: false) else {
            throw HotChatError.generalError(reason: .conversionError(string: string, encoding: .utf8))
        }
        try set(data, for: key)
    }
    ...
    func keychainError(_ status: OSStatus) -> HotChatError {
    ...
    func data(for key: String) throws -> Data? {
        ...
        case errSecSuccess:
            guard let data = result as? Data else {
                throw HotChatError.authorizeFailed(reason: .invalidDataInKeychain)
            }
            return data
