/******************************************************************************
 *
 *  本文件声明了 TextViewDelegate 协议和 InputBar 类。
 *  TUI 输入条，用于检测、获取用户输入的 UI 组件。
 *  InputBar，即位于聊天消息最下方的 UI 组件。包括文本输入框、表情按钮、语音按钮和“+”按钮（“更多”按钮）。
 *  TextViewDelegate 提供了输入条各种情况下的回调委托，包括点击输入条的表情、“更多”视图、语音按钮的回调。以及发送消息、发送语音、更改输入高度的回调。
 *
 ******************************************************************************/
...
*  @param textView 委托者，当前与用户交互的 InputBar。
*/
- (void)inputBar:(InputBar *)textView didDeleteAt:(NSString *)text;
