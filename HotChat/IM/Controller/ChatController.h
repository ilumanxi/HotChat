 ...
 *
 * ChatController 类用于实现聊天视图的总控制器，负责将输入、消息控制器、更多视图等进行统一控制。
 * 本文件中声明的类与协议，能够有效的帮助您实现自定义消息格式。
 *
 * ChatControllerDelegate 负责提供聊天控制器的部分回调委托。包括接收新消息、显示新消息和某一“更多”单元被点击的回调。
 *
 ******************************************************************************/
 ...
 *  - 如果您返回 nil，ChatController 会认为该条消息非自定义消息，会将其按照普通消息的处理流程进行处理。
 *  - 如果您返回一个 TUIMessageCellData 类型的对象，ChatController 会在随后触发的 onShowMessageData() 回调里传入您返回的 cellData 对象。
 *
 *  也就是说，onNewMessage() 负责让您甄别自己的个性化消息，而 onShowMessageData() 回调则负责让您展示这条个性化消息。
 *
 * <pre>
 *  - (TUIMessageCellData *)chatController:(ChatController *)controller onNewMessage:(TIMMessage *)msg
 *  {
 ...
 */
- (TUIMessageCellData *)chatController:(ChatController *)controller onNewMessage:(V2TIMMessage *)msg;
 ...
 *
 * <pre>
 *  - (TUIMessageCell *)chatController:(ChatController *)controller onShowMessageData:(TUIMessageCellData *)data
 *  {
 ...
 */
- (TUIMessageCell *)chatController:(ChatController *)controller onShowMessageData:(TUIMessageCellData *)cellData;
 ...
 * <pre>
 *  - (void)chatController:(ChatController *)chatController onSelectMoreCell:(TUIInputMoreCell *)cell{
 ...
 *  @param cell 所点击的消息单元
 */
- (void)chatController:(ChatController *)controller onSelectMessageAvatar:(TUIMessageCell *)cell;
 ...
 *  @param cell 所点击的消息单元
 */
- (void)chatController:(ChatController *)controller onSelectMessageContent:(TUIMessageCell *)cell;
...
/**
 * 【模块名称】聊天界面组件（ChatController）
 *
 * 【功能说明】负责实现聊天界面的 UI 组件，包括消息展示区和消息输入区。
 *
 *  ChatController 类用于实现聊天视图的总控制器，负责将聊天消息控制器（TUIMessageController）、信息输入控制器（InputController）和更多视频进行统一控制。
 *
 *  聊天消息控制器负责在您接收到新消息或者发送消息时在 UI 作出响应，并响应您在消息气泡上的交互操作，详情参考:Section\Chat\TUIMessageController.h
 *  信息输入控制器负责接收您的输入，向你提供输入内容的编辑功能并进行消息的发送，详情参考:Section\Chat\Input\InputController.h
 ...
 */
@interface ChatController : UIViewController<InputControllerDelegate>
 ...
 *  同时 InputController 整合了消息的发送功能，您可以直接使用 InputController 进行消息的输入采集与发送。
 *  信息输入控制器的详细信息请参考 Section\Chat\Input\InputController.h
 */
 ...
 *  需要继续监听 ChatControllerDelegate 中的 onNewMessage 和 onShowMessageData 回调才能完成个性化消息的展示。
 *
