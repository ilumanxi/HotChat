 ...
 *
 * 本文件声明了实现输入区的相关组件。
 * 输入区即包括表情视图输入区（TUIFaceView+TUIMoreView）、“更多”功能区（TUIMoreView）和文本输入区（InputBar）。
 * 本文件包含InputControllerDelegate 协议和 TInputController 类。
 * 在输入条（InputBar）中，提供了表情、语音、更多视图的按钮响应委托。
...
/**
* 【模块名称】InputController
* 【功能说明】TUI 输入控制器，实现了输入条中进一步的业务逻辑。
*  在输入条（InputBar）中，提供了表情、语音、更多视图的按钮和响应回调。
*  而在本类中，将 InputBar 与上述三个视图实际结合，实现了各个视图的显示与切换的逻辑。
*/
...
*  详细信息请参考 Section\Chat\Input\InputBar.h
*/
...
/**
*  实现 InputControllerDelegate 协议的委托。
*/
